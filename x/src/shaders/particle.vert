...
// Easter egg heart
uniform float uEasterHeartStart; // seconds, -1 inactive
uniform float uEasterHeartDur;   // seconds duration
uniform float uEasterHeart;      // 0..1 envelope
...
void main() {
  ...
  if (uCollapsePhase > 0.0) {
    ...
    float shrink = mix(1.0, 0.02, smoothstep(0.0, 1.0, cp));
    pos *= shrink;
    // If heart easter egg active, morph collapsed cluster into heart silhouette in XY
    if (uEasterHeart > 0.001) {
      // Map current pos (already near center) to a normalized domain
      vec2 p = pos.xy * (55.0); // scale factor to spread tiny cluster into heart space
      // Inverse mapping not needed; just push points outward to heart SDF zero set
      float x = p.x; float y = p.y;
      float f = pow(x*x + y*y - 1.0, 3.0) - x*x*y*y*y; // heart implicit
      // approximate gradient
      float e = 0.002;
      float fx = ( (pow((x+e)*(x+e) + y*y - 1.0, 3.0) - (x+e)*(x+e)*y*y*y) - (pow((x-e)*(x-e) + y*y - 1.0, 3.0) - (x-e)*(x-e)*y*y*y) ) / (2.0*e);
      float fy = ( (pow(x*x + (y+e)*(y+e) - 1.0, 3.0) - x*x*(y+e)*(y+e)*(y+e)) - (pow(x*x + (y-e)*(y-e) - 1.0, 3.0) - x*x*(y-e)*(y-e)*(y-e)) ) / (2.0*e);
      vec2 g = vec2(fx, fy);
      float gl = length(g)+1e-6;
      vec2 n = g / gl;
      // Move toward surface f=0 with small step scaled by envelope
      float stepK = 0.18;
      p -= n * f * stepK * uEasterHeart;
      // breathing scale pulsation
      float breathe = 1.0 + 0.15 * sin(uTime*4.0) * uEasterHeart;
      p *= breathe;
      // write back (inverse of initial scale)
      pos.xy = p / 55.0;
      // slight lift for style
      pos.y += 0.004 * uEasterHeart;
    }
  }
